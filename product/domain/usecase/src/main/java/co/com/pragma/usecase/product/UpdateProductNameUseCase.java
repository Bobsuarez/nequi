...
public class UpdateProductNameUseCase {
    ...
    public Mono<Product> execute(ProductId productId, String name) {
        if (name == null || name.isBlank()) {
            return Mono.error(new BusinessException("El nombre no puede estar vacío"));
        }
        return Mono.defer(() ->
                productRepository.findById(productId)
                        ...
                        .map(p -> p.withName(name.trim()))
                        .flatMap(updated -> productRepository.update(updated, co.com.pragma.model.product.ProductRenamedEvent.of(updated)))
        );
    }
}
